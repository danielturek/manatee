...
## add slice samplers for pDet[1:6, 1]
for(n in fraction.model.calf$expandNodeNames('pDet[1:6, 4]')) {
    fraction.mcmcConf.calf$removeSamplers(n)
    fraction.mcmcConf.calf$addSampler(target = n, type = 'slice')
}
fraction.mcmcConf.calf$printSamplers()
