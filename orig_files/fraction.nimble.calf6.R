# Packages
library(plyr); library(VGAM); library(nimble); library(igraph); library(coda)
...
base.mort <- array(c(1 - sqrt((1 - calf1.mort.ratio * adult.base.mort) * (1 - calf2.mort.ratio * adult.base.mort)),
                     adult.base.mort, adult.base.mort), c(nRegions, nClasses),
                   dimnames = list(regions, classes))

data <- transform(mdata, area = factor(area, levels=regions), age=factor(age, levels=classes),
                  habitat = factor(habitat, levels = habitats))

  
# Constants
CALF <- 1; SUBADULT <- 2; ADULT <- 3
...
coldDesignations <- matrix(c("Cold", rep("Normal", 4), "Cold", "Normal", "Cold", rep("Normal", 6), "Severe", "Cold", rep("Normal", 2),
...
data.fraction.calf5 <- list(data1=data.array[ , 'Calves', , , 1:nCauses], nCauses=nCauses, nRegions=nRegions, 
                           ...
                           totals = apply(data.array[ , 'Calves', , , 1:nCauses], 1:3, sum), # When model is set up for total carcasses, remove "1:nCauses"
...
# Nimble model
inits5 <- inits.calf5()
fraction.model5 <- nimbleModel(fraction.code5, constants = data.fraction.calf5,
                               inits = inits5)
...
fraction.mcmcConf5$resetMonitors()
fraction.mcmcConf5$addMonitors('pi')
fraction.mcmcConf5$addMonitors('p')
fraction.mcmcConf5$addMonitors('tide_mort')
fraction.mcmcConf5$addMonitors('cold_mort')

fractionMCMC5 <- buildMCMC(fraction.mcmcConf5)
CfractionMCMC5 <- compileNimble(fractionMCMC5, project = fraction.model5, resetFunctions = T)

#Run the model
print(system.time(CfractionMCMC5$run(500000)))

# Examine results
sample.mat <- as.matrix(CfractionMCMC5$mvSamples)
summary(sample.mat)
colnames(sample.mat)
sample.mcmc <- as.mcmc(sample.mat[200001:nrow(sample.mat), -c(3, 6, 21, 45, 58)]) # Remove parameters fixed at or near 0
geweke.diag(sample.mcmc)
geweke.plot(sample.mcmc, ask = F)
